...
class P {
  ...
  filter(t, i) {
    ...
    const { xPrev: e, tPrev: o, dxPrev: s } = this,
      r = t - o,
      u = h(r, this.dCutOff),
      c = [],
      d = [],
    ...
    for (let a = 0; a < i.length; a++) {
      ...
      const m = this.minCutOff + this.beta * Math.abs(d[a]),
        f = h(r, m);
      l[a] = p(f, i[a], e[a]);
    }
...
class k {
  constructor({ uiLoading: t, uiScanning: i, uiError: e }) {
    ...
    (o.innerText = b),
      ...
      t === "yes"
        ? (this.loadingModal = this._loadHTML(g))
        : t !== "no" && (this.loadingModal = document.querySelector(t)),
      e === "yes"
        ? (this.compatibilityModal = this._loadHTML(v))
        : e !== "no" && (this.compatibilityModal = document.querySelector(e)),
      i === "yes"
        ? (this.scanningMask = this._loadHTML(y))
...
export { P as O, k as U, w as a, x as c, M as g };
